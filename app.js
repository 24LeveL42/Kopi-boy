...
async function ensureSession(){
  if(!window.KOPI_SUPABASE_READY||typeof supabase==="undefined"){toast("Kopi Boy is not connected");return null;}
  const {data,error}=await supabase.auth.getSession();
  if(error){console.error(error);toast("Could not check your login");return null;}
  if(data.session)return data.session;
  kbOpenAuth("Sign in to Kopi Boy");
  return null;
}
...
async function kbRefreshProfile(){
  if(!kbAuthReady())return;
  const {data:{session}}=await supabase.auth.getSession();
  const card=document.querySelector("#customerProfileScreen .profile-card");
  if(!card)return;
  const h2=card.querySelector("h2"), p=card.querySelector("p");
  let signout=card.querySelector(".kb-signout");
  if(session?.user){
    const user=session.user, name=user.user_metadata?.full_name||user.user_metadata?.name||user.email?.split("@")[0]||"Customer";
    if(h2)h2.textContent=name;
    if(p)p.innerHTML=`Signed in as <b>${esc(user.email||"")}</b>`;
    if(!signout){signout=document.createElement("button");signout.className="text-orange full kb-signout";signout.textContent="SIGN OUT";signout.onclick=kbSignOut;card.appendChild(signout);}
  }else{
    if(h2)h2.textContent="Customer";
    if(p)p.textContent="Sign in to manage your account, addresses and orders.";
...
async function kbBootstrapAuth(){
  if(!kbAuthReady())return;
  const {data:{session}}=await supabase.auth.getSession();
  if(session){kbCloseAuth();await kbRefreshProfile();return;}
  kbOpenAuth("Sign in to Kopi Boy");
  supabase.auth.onAuthStateChange(async(event,newSession)=>{
    if(event==="SIGNED_IN"&&newSession){
      kbCloseAuth();
      await kbRefreshProfile();
      loadCooks();
      toast("Welcome to Kopi Boy ✓");
...
document.addEventListener("DOMContentLoaded",async()=>{loadCustomerLocation();kbBootstrapAuth();});
