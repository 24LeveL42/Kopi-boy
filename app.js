...
function placeOrder(){if(!customerConn?.open){toast("Connect to the Cook first");return;}const order={id:"KB"+Math.floor(10000+Math.random()*89999),createdAt:Date.now(),items:cart.map(x=>({name:x.name,price:x.price})),total:cartTotal()+2.5,status:"placed"};testOrder=order;$("orderNumber").textContent="Order #"+order.id;$("confirmedSeller").innerHTML=`<img src="${currentCook.avatar}"><div><b>${currentCook.name}</b><small style="display:block;color:#888">${currentCook.time} · ${currentCook.distance}</small></div>`;customerConn.send({type:"new_order",order});renderCustomerOrderBanner();cart=[];go("orderConfirmed");toast("Order sent to Cook ✓");}
function setupCook(){
